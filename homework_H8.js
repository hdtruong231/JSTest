...
const organizationTree = {
  ...
  let obj = [];
  let currentID = 1;
  function convertreetoflat(organizationTree, position = "" ,parentId = null)
  {
   let newObj = {
    id: currentID,
    name: organizationTree.name,
    position: position ? organizationTree.name  + " > " + position : organizationTree.name,
    parentId: parentId
  };
  obj.push(newObj);
  currentID++;
    if (organizationTree.children)
    {
        organizationTree.children.forEach(function(child) {
            convertreetoflat(child, newObj.position, newObj.id);
        });
    }
  }
  convertreetoflat(organizationTree,"", null);
  console.log(obj);
